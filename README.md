...
Для сборки и запуска базы и сервиса: `docker compose up`
Для прогона тестов: 

* запустить только базу `docker compose up db`
* добавить в окружение `DB_URL=postgres://postgres:secret@localhost:5432/postgres` и `ALLOW_ERASE_DATABASE_CONTENT=yes`
* установить зависимости `make dep`, и разово запустить и остановить сервис: `make run` (накатывает миграции)
* и запустить тесты `make test`
