...
func TestUnmarshalInput(t *testing.T) {
	...
	tests := []struct {
		...
		{"no content type", args{httptest.NewRequest("POST", "/top-up", strings.NewReader("")), nil}, assertErrorContains("content-type header required"), func(t *testing.T, message any) { assert.Nilf(t, message, "message want nil, got %v", message) }},
		{"ct json, body empty", args{withContentTypeJson(httptest.NewRequest("POST", "/top-up", bytes.NewReader([]byte{}))), &proto.TopUpInput{}}, assertErrorContains("unmarshal json"), nil},
		{"ct json, body invalid", args{withContentTypeJson(httptest.NewRequest("POST", "/top-up", strings.NewReader("xxx"))), &proto.TopUpInput{}}, assertErrorContains("unmarshal json"), nil},
		{"ct json, body invalid", args{withContentTypeJson(httptest.NewRequest("POST", "/top-up", strings.NewReader("xxx"))), &proto.TopUpInput{}}, assertErrorContains("unmarshal json"), nil},
		{"ct json, top-up input", args{withContentTypeJson(httptest.NewRequest("POST", "/top-up", strings.NewReader(`{"user_id": "anna","currency":"USD","value":"10.00"}`))), &proto.TopUpInput{}}, assert.NoError, func(t *testing.T, message any) {
		...
		{"ct protobuf, body empty", args{withContentTypeProtobuf(httptest.NewRequest("POST", "/top-up", bytes.NewReader([]byte{}))), &proto.TopUpInput{}}, assert.NoError, func(t *testing.T, message any) {
